...
def index(request):
    ...
    return JsonResponse({'poems': poems_data, 'total_pages': paginator.num_pages, 'current_page': page}, safe=False)
...
def get_poem(request, pk):
    try:
        ...
        return JsonResponse({'error': 'Poem does not exist'}, safe=False, status=HTTPStatus.NOT_FOUND)
    poem_response = {
        'id': poem.id,
        'title': poem.title,
        'author': {
